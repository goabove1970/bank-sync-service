import { BankConnectionResponse, BankSyncArgs, BankSyncRequestType } from "./connections-request";
...
import { BankPollController } from '../controllers/bank-controller';
...
import { BankSyncScheduler } from "../controllers/bank-controller/scheduler";
import logger from "../logger";
import { toResponseBankConnection } from "./connection-route-utils";

export class ConnectionsRequestProcessor {
    bankConnectionsControlelr: BankConnectionController;
    pollController: BankPollController;
    accountController: AccountController;
    scheduler: BankSyncScheduler;
    constructor(bankConnectionsControlelr: BankConnectionController,
            pollController: BankPollController,
            accountController: AccountController,
            scheduler: BankSyncScheduler) {
    ...
    async processAddBankConnectionRequest(args: BankSyncArgs): Promise<BankConnectionResponse> {
        ...
        const connections = await this.bankConnectionsControlelr.read({ userId: args.userId });
        ...
        try {
          const connectionStatus = await this.pollController.getConnectionStatus(newBankConnection);
      ...
      async processReadBankConnectionsRequest(args: BankSyncArgs): Promise<BankConnectionResponse> {
        const response: BankConnectionResponse = {
        ...
        try {
          let connections = await this.bankConnectionsControlelr.read({ userId: args.userId });
      ...
      async processRemoveBankConnectionRequest(args: BankSyncArgs): Promise<BankConnectionResponse> {
        const response: BankConnectionResponse = {
        ...
        try {
          const connections = await this.bankConnectionsControlelr.read({ connectionId: args.connectionId });
          ...
          await this.bankConnectionsControlelr.delete({ connectionId: args.connectionId });
          response.payload = {
            ...response.payload,
      ...
      async processUpdateBankConnectionRequest(args: BankSyncArgs): Promise<BankConnectionResponse> {
        const response: BankConnectionResponse = {
        ...
        try {
          ...
          const connectionStatus = await this.pollController.getConnectionStatus(connection);
          if (connectionStatus.statusData && connectionStatus.statusData.severity !== 'ERROR') {
      ...
      async processSyncBankConnectionsRequest(args: BankSyncArgs): Promise<BankConnectionResponse> {
        const response: BankConnectionResponse = {
        ...
        try {
          const syncData = await this.scheduler.executeSync(args.userId, args.connectionId, true);
