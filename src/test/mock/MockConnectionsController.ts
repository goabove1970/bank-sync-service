...
import { validateConnectionCreateArgs } from '@root/src/controllers/data-controller/bank-connection/helper';
...
const MockCreateBankConnection = jest.fn((args: BankConnection): Promise<void> => {
    validateConnectionCreateArgs(args);
    getCollection().push(args);
    return Promise.resolve();
});

const MockReadBankConnection = jest.fn((args: BankSyncArgs): Promise<BankConnection[]> => {
...
const MockUpdateBankConnection = jest.fn((args: BankSyncArgs): Promise<void> => {
    validateConnectionCreateArgs(args);
    updateItem(args);
    return Promise.resolve();
});

const MockDeleteBankConnection = jest.fn((args: BankSyncArgs): Promise<void> => {
    deleteItem(args.connectionId);
    return Promise.resolve();
});

export let MockBankConnectionController = jest.fn<BankConnectionController, []>(() => ({
