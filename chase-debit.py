#!/usr/bin/python
import time, os, http.client, urllib.request, urllib.error, urllib.parse, uuid
import sys
...
# this is from _ccreq below and reading page 176 of the latest OFX doc.
    ...
    def doQuery(self,query,name):
        ...
        garbage, path = urllib.parse.splittype(self.config["url"])
        host, selector = urllib.parse.splithost(path)
...
if __name__=="__main__":
    ...
    if len(argv) < 4:
        ...
        print("available sites:",join(", ",list(sites.keys())))
        sys.exit()
    passwd = argv[3]
